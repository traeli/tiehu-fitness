...
本文约定铁虎健身教练后端新增接口时的标准开发顺序。目标是让微信小程序使用 HTTP/JSON，让微服务之间使用 gRPC，同时复用同一套业务逻辑。
...
| 服务 | 业务职责 | 示例 |
|---|---|---|
| `core-service` | 登录会话、用户档案、器材动作内容、训练计划、训练记录和打卡 | 微信登录、查询器材、生成计划、完成训练 |
