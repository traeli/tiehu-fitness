...
实时 PCM 分片生成后立即发送，不在 Renderer 中累计实时转写队列。当前留存录音由 MediaRecorder 压缩并设置 256 MB 上限，会议正常停止后通过 preload 文件适配器写入“文稿/会议助手/录音”；本地索引只保存受校验的会议与文件元数据。长会议的录制过程流式 spool 和崩溃恢复仍是下一阶段可靠性工作。
